...
def createPDFDocumentWithPath(path):
	global verbose
	if verbose:
		print "[Python] . Creating PDF document from file %s" % (path)
...
def writePageFromDoc(writeContext, doc, pageNum):
	...
	if page:
		...
		if verbose:
			print "[Python] . Copied page %d from %s" % (pageNum, doc)
...
def main(argv):
	...
	try:
		options, args = getopt.getopt(argv, "o:sv", ["output=", "verbose"])
	...
	for option, arg in options:
		...
		if option in ("-o", "--output") :
			if verbose:
				print "[Python] . Setting %s as the destination." % (arg)
			...
			print "[Python] . Verbose mode enabled."
			verbose = True
...
def usage():
	print "[Python] . Usage: join [--output <file>] [--verbose]"
